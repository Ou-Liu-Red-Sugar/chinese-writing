...
现行版本 **2.12.3** 明确[学习笔记的讲述口吻](references/sentence-construction.md#6-教材语气与作者引导)：让作者与读者一起观察、比较、追问和推理，主动使用有作用的“我们”等共同主语与承接，保留读者跟随解释的过程。第0版安排阅读进程，第1版写出真实引导，审读同时核事实理解与讲述关系；报告和参考记录仍按各自用途选择声音。方法来自本次用户反馈与编辑归纳，[示范](assets/sentence-calibration.md#七学习笔记的共同主语与讲述关系)保留其实际身份，不以代词数量或未运行的测试证明效果。
