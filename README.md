...
现行版本 **2.13.1** 将案例自身是否自然成立接回[选材与详略](references/composition.md#2-选材与详略)：核情境、动作来由及额外条件的理解成本。用于核算的封闭题设仍可使用；进入可读笔记时，材料拼接造成的生硬要回到选材处理，不能只靠句法或标点修订。

[真实样文的标点与句群校准](references/punctuation-profiles.md)保留五篇中文文章的固定版本、正文范围和初次测量；用实际句群解释数字，再在新材料中试写、审读和修订。统计辅助定位，不按符号数量判定文风，也不将这五篇当作中文总体的最佳比例。
