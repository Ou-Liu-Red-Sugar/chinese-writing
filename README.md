...
现行版本 **2.10.1** 校准连接词的取舍：同时考虑语义、语气和阅读节奏，检查衔接不足与提示堆叠，不以少用为目标。资料工作继续分为[取材与深挖](references/workflow.md#取材与深挖)。取材服务第0版和第1版草图，允许粗读概述、摘要、定义或短片段；足以决定主线与呈现办法就停，不逐表取数或完成整套核查。第1版正式交出并接收后，再按草图需要深挖原件、数据、条件和计算。
